...
function getFilesystemStoreDependencies() {
  ...
  try {
    const storeModule = require("../state/extensionStateStore");
...
function readTextFileSafe(filePath) {
  const result = readTextFilePath(filePath);
  if (result === null) {
    console.error(`[algorithms-runner] readTextFileSafe failed for "${filePath}"`);
    return "";
  }
  return result;
}
