...
const runControlsUiClientUtils = window.AlgosWebviewClientUtils;
...
try {
  if (!runControlsUiClientUtils) {
    throw new Error("Run Controls missing AlgosWebviewClientUtils bootstrap.");
  }

  requiredNodes = runControlsUiClientUtils.getRequiredNodes(
    REQUIRED_NODE_IDS,
    "Run Controls"
  );
} catch (error) {
  ...
  if (runControlsUiClientUtils) {
    runControlsUiClientUtils.showStartupFailure("Run Controls", failureText);
  }
...
runArgsEnabled.addEventListener("change", () => {
  ...
  window.AlgosRunControlsComms.sendSetEnabled(enabled);
});

runArgsText.addEventListener("input", () => {
  updateRunArgsClearButtonVisibility();
  window.AlgosRunControlsComms.sendSetText(runArgsText.value);
});

clearRunArgs.addEventListener("click", () => {
  runArgsText.value = "";
  window.AlgosRunControlsComms.sendSetText("");
...
sourceProfileEnabled.addEventListener("change", () => {
  ...
  window.AlgosRunControlsComms.sendSetSourceProfileEnabled(enabled);
});

sourceProfileText.addEventListener("input", () => {
  updateSourceProfileClearButtonVisibility();
  window.AlgosRunControlsComms.sendSetSourceProfileText(sourceProfileText.value);
});

clearSourceProfile.addEventListener("click", () => {
  sourceProfileText.value = "";
  window.AlgosRunControlsComms.sendSetSourceProfileText("");
...
runChecksModeNone.addEventListener("change", () => {
  ...
  updateRunChecksRouteInteractivity();
  window.AlgosRunControlsComms.sendSetRunChecksMode("none");
});
...
runChecksModeCheckOnly.addEventListener("change", () => {
  ...
  updateRunChecksRouteInteractivity();
  window.AlgosRunControlsComms.sendSetRunChecksMode("check-only");
});
...
runChecksModeCompileOnly.addEventListener("change", () => {
  ...
  updateRunChecksRouteInteractivity();
  window.AlgosRunControlsComms.sendSetRunChecksMode("compile-only");
});

runChecksRoute.addEventListener("change", () => {
  window.AlgosRunControlsComms.sendSetRunChecksRoute(runChecksRoute.value);
});

cleanStdlibEnabled.addEventListener("change", () => {
  window.AlgosRunControlsComms.sendSetCleanStdlibEnabled(cleanStdlibEnabled.checked);
});

cleanArchivesEnabled.addEventListener("change", () => {
  window.AlgosRunControlsComms.sendSetCleanArchivesEnabled(cleanArchivesEnabled.checked);
});

window.AlgosRunControlsUi = {
  applyState,
};
