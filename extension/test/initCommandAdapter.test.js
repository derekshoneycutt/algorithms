...
/**
 * Creates one successful spawn mock with deterministic output.
 *
 * @param {string} stdoutText stdout text.
 * @returns {(command: string, args: string[], options: object) => EventEmitter} Spawn mock.
 */
function createSpawnSuccessMock(stdoutText) {
  return () => {
    ...
    process.nextTick(() => {
      stdoutEmitter.emit("data", String(stdoutText || ""));
...
async function testExecuteInitOperationCheckEnvSuccess() {
  ...
  const status = await executeInitOperation({
    ...
    profilePath: "",
    spawnFn: createSpawnSuccessMock("all good"),
