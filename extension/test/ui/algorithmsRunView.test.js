...
const { getLanguageDisplayLabel } = require("../../src/runtime/language/languageModule");
const {
  withPatchedModuleLoad,
} = require("../__helpers/withPatchedModuleLoad");
const {
...
function loadAlgorithmsRunViewInternals() {
  return withPatchedModuleLoad((request, parent, isMain, originalModuleLoad) => {
    if (request === "vscode") {
      return {
        Uri: {
          file(filePath) {
